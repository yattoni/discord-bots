...
Mention the bot with `help` (for example `@stock-bot help`) to get a short explanation of how it works. Mention it with any other question — including ones unrelated to stocks — and it replies using Gemma 4 31B Instruct (`google/gemma-4-31b-it`) through OpenRouter. Messages the bot processes are written to its logs.
