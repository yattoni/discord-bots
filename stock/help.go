...
- Add a range after the ticker: ` + "`$NOW 1W`" + `, ` + "`$AAPL 2W`" + `, or ` + "`$BTC YTD`" + `
...
- Tickers are 1–5 letters, with an optional share class like ` + "`$BRK.B`" + `, a quote currency like ` + "`$BTC-USD`" + `, or a caret index like ` + "`$^TNX`" + `
- ` + "`$BTC`" + ` is Bitcoin spot (same as ` + "`$BTC-USD`" + `)
- Extra text around the ticker is not allowed — the message must be just the ticker, or the ticker plus a range (whitespace is fine). If I don't recognize a range I'll list the ones I accept.
