...
func TestHelpMessageDocumentsBitcoinSpot(t *testing.T) {
	...
	assert.Contains(t, helpMessage, "$NOW 1W")
	assert.Contains(t, helpMessage, "$AAPL 2W")
	assert.Contains(t, helpMessage, "$BTC YTD")
	assert.Contains(t, helpMessage, "5D")
	assert.Contains(t, helpMessage, "1W")
	assert.Contains(t, helpMessage, "2W")
	assert.Contains(t, helpMessage, "1M")
	assert.Contains(t, helpMessage, "2Y")
	assert.Contains(t, helpMessage, "5Y")
	assert.Contains(t, helpMessage, "YTD")
	assert.Contains(t, helpMessage, "don't recognize a range")
	assert.Contains(t, helpMessage, "$^TNX")
	assert.Contains(t, helpMessage, "$^GSPC")
	assert.Contains(t, helpMessage, "$^VIX")
	assert.Contains(t, helpMessage, "caret")
